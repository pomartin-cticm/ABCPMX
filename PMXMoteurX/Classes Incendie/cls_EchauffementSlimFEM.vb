Public Class cls_EchauffementSlimFEM
    '===== CLASSE POUR LE CALCUL NUMERIQUE DE L'ECHAUFFEMENT D'UNE POUTRE DE PLANCHER MINCE ======

#Region " Solveur Numérique "


    Public Sub Calcul_thermique_Poutre_plancher_mince(ByRef my_Mesh As cls_MaillageSlimFloor, myParamFeu As cls_OptionsFeu,
                                                  my_Time As Double, DeltaT As Double, lTargetT As Boolean, TempG As Double, val_U As Decimal,
                                                  lNormal As Boolean, lANFrance As Boolean, lGeneration1 As Boolean,
                                                  RhoC As Decimal, lRhoCVariable As Boolean)
        '---------------------------------------------------------------------------------------------------------------------------------------
        '   14/11/25 :  Création - GiB
        '---------------------------------------------------------------------------------------------------------------------------------------
        '   Calcul de l'échauffement d'une dalle en béton exposée à l'incendie normalisé en sous-face
        '---------------------------------------------------------------------------------------------------------------------------------------
        '   my_Mesh         [E] :   Classe de caractéristiques du maillage (taille, matériaux et températures)
        '   My_fire         [E] :   Classe de caractéristiques du calcul au feu
        '   My_time         [E] :   Temps actuel du calcul (s) (étape précécente, t=my_Time-DeltaT)
        '   DeltaT          [E] :   Pas de temps du calcul (s)
        '   lTargetT        [E] :   indique si le temps de calcul est inférieur à la valeur cible (true) ou s'il correspond à la valeur cible (false)
        '   TempG           [E] :   Température des gaz chauds au contact de la face exposée de la dalle (°C)
        '   val_U           [E] :   taux d'humidite (%)
        '   lNormal         [E] :   indique si béton NC (true) ou LC (false)
        '   lGeneration1    [E] :   indique si génération 1 ou génération 2 des Eurocodes
        '   lANFrance       [E] :   indique si on utilise l’Annexe Nationale française de l’EN 1994-1-2
        '   RhoC            [E] :   masse volumique du béton (à froid)                              (kg/m3)
        '   lRhoCVariable   [E] :   indique si on utilise la formule de rhoc de l’EN 1994-1-2 variant en fonction de la température, pour un béton normal.
        '                           Si non, on utilise la valeur constante du paramètre RhoC.
        '                           Pour le béton léger, on utilise toujours RhoC.
        '---------------------------------------------------------------------------------------------------------------------------------------
        '   Paramètre en sortie:
        '   My_mesh.Tab_mesh_temp(i,j)       Température dans chaque maille (i,j) de la poutre, au temps t + DeltaT  (°C)
        '---------------------------------------------------------------------------------------------------------------------------------------



        '--( Déclarations

        Dim EN_Feu As New cls_EurocodesFeu

        '%% Temperature des mailles
        'Dim Temp_0(0 To my_Mesh.nb_cells_y - 1, 0 To my_Mesh.nb_cells_z - 1) As Double
        'Dim Temp_1(0 To my_Mesh.nb_cells_y - 1, 0 To my_Mesh.nb_cells_z - 1) As Double
        Dim Temp_0(0 To my_Mesh.ind_1 - my_Mesh.ind_2, 0 To my_Mesh.nb_cells_z - 1) As Double
        Dim Temp_1(0 To my_Mesh.ind_1 - my_Mesh.ind_2, 0 To my_Mesh.nb_cells_z - 1) As Double

        '%% Parametres variables de la boucle de calcul
        Dim rho_ As Double              ' masse volumique du materiau d'une maille a un instant donne
        Dim cp_ As Double               ' chaleur specifique du materiau d'une maille a un instant donne
        Dim rho_cp As Double            ' produit rho * cp du materiau d'une maille a un instant donne
        Dim lambda_ As Double           ' conductivite thermique du materiau d'une maille a un instant donne
        Dim val_dth As Double           ' increment de temperature d'une maille pendant val_DeltaT
        Dim h_net_ce As Double          ' densite de flux convectif sur les faces exposees a un instant donne
        Dim h_net_re As Double          ' densite de flux radiatif sur les faces exposees a un instant donne
        Dim h_net_de As Double          ' densite totale de flux sur les faces exposees a un instant donne
        Dim h_net_cn As Double          ' densite de flux convectif sur les faces non exposees a un instant donne
        Dim h_net_rn As Double          ' densite de flux radiatif sur les faces non exposees a un instant donne
        Dim h_net_dn As Double          ' densite totale de flux sur les faces non exposees a un instant donne

        Dim val_epsilon As Double       ' emissivite de surface d'une face exterieure de maille

        Dim dth_lb As Double, dth_bb As Double, dth_tb As Double, dth_rb As Double

        Dim h_lb_1 As Double, h_bb_1 As Double, h_tb_1 As Double, h_rb_1 As Double
        Dim h_lb_2 As Double, h_bb_2 As Double, h_tb_2 As Double, h_rb_2 As Double

        Dim lambda_Lb As Double, lambda_Bb As Double, lambda_Tb As Double, lambda_Rb As Double

        Dim k_1 As Double
        Dim q_lb As Double, q_bb As Double, q_tb As Double, q_rb As Double, q_y As Double, q_z As Double
        Dim cst_lb As Double, cst_bb As Double, cst_tb As Double, cst_rb As Double

        Dim i_ As Integer, j_ As Integer, i_mat As Integer, j_mat As Integer, ii_ As Integer, jj_ As Integer, i_y As Integer, ii_y As Integer
        Dim prop_expo As Boolean
        Dim prop_noex As Boolean

        Dim NbY As Integer = my_Mesh.nb_cells_y
        Dim NbZ As Integer = my_Mesh.nb_cells_z

        Dim SigmaSB As Double = cls_OptionsFeu.BOLTZMANN
        Dim AlphaCInf As Double, AlphaCSup As Double
        Dim EpsilonF As Double

        Dim i_mat_a_0 As Integer    'GiB 18/11/2025 : ajout
        Dim i_mat_a_1 As Integer    'GiB 18/11/2025 : ajout
        Dim i_mat_c As Integer    'GiB 18/11/2025 : ajout
        Dim i_mat_v_0 As Integer    'GiB 01/12/2025 : ajout

        '--( Initialisation 

        AlphaCInf = myParamFeu.ConvectionCoef
        AlphaCSup = myParamFeu.ConvectionCoefDalle
        EpsilonF = myParamFeu.EmissivityFire

        i_mat_a_0 = my_Mesh.MATACIERPLAT
        i_mat_a_1 = my_Mesh.MATARMA
        i_mat_c = my_Mesh.MATBETON
        i_mat_v_0 = my_Mesh.MATVIDEOUVERT

        'GiB 20/11/2025 : cohérence avec le reste du code : les conditions d'égalité ou d'inégalité sont remplacées par les fonctions IsSmaller, IsEqual, etc.

        '--( Temperature des mailles à l'instant val_t

        'For i_ = 0 To NbY - 1
        For i_ = my_Mesh.ind_2 To my_Mesh.ind_1
            i_y = i_ - my_Mesh.ind_2
            For j_ = 0 To NbZ - 1
                'Temp_0(i_, j_) = my_Mesh.Tab_mesh_temp(i_, j_)
                'Temp_1(i_, j_) = Temp_0(i_, j_)
                Temp_0(i_y, j_) = my_Mesh.Tab_mesh_temp(i_, j_)
                Temp_1(i_y, j_) = Temp_0(i_y, j_)
            Next
        Next

        '--( Boucle sur les mailles entre ind_0 et ind_1 pour calculer la temperature de chacune à l'instant val_t + val_DeltaT

        For j_ = 0 To NbZ - 1

            For i_ = my_Mesh.ind_2 To my_Mesh.ind_1

                'q_lb : échange entre (i_-1, j_) et (i_,j_) : "left boundary"
                'q_tb : échange entre (i_, j_-1) et (i_,j_) : "top boundary"
                'q_bb : échange entre (i_, j_-1) et (i_,j_) : "bottom boundary"
                'q_rb : échange entre (i_+1, j_) et (i_,j_) : "right boundary"

                'q_y : q_lb + q_rb
                'q_z : q_bb + q_tb

                '%%Initialisation de valeurs
                val_dth = 0.0
                dth_lb = 0.0 : dth_bb = 0.0 : dth_tb = 0.0 : dth_rb = 0.0
                h_lb_1 = 0.0 : h_bb_1 = 0.0 : h_tb_1 = 0.0 : h_rb_1 = 0.0
                h_lb_2 = 0.0 : h_bb_2 = 0.0 : h_tb_2 = 0.0 : h_rb_2 = 0.0

                'Indice de ligne des tableaux Temp_0 et Temp_1
                i_y = i_ - my_Mesh.ind_2

                '%%Numero de materiau de la maille (i_,j_)
                i_mat = my_Mesh.Tab_mesh_mat(i_, j_)

                '%%Conditions d'exposition au feu
                prop_expo = False
                If IsGreaterOrEqual(i_mat, i_mat_a_0) Then

                    If IsSmaller(j_, Temp_0.GetUpperBound(1)) AndAlso IsEqual(my_Mesh.Tab_mesh_mat(i_, j_ + 1), i_mat_v_0) Then
                        prop_expo = True
                    End If

                    If (Not prop_expo) AndAlso (IsEqual(j_, Temp_0.GetLowerBound(1)) OrElse IsEqual(my_Mesh.Tab_mesh_mat(i_, j_ - 1), i_mat_v_0)) Then
                        prop_expo = True
                    End If

                    If (Not prop_expo) AndAlso (IsGreater(i_, Temp_0.GetLowerBound(0) + my_Mesh.ind_2) AndAlso IsEqual(my_Mesh.Tab_mesh_mat(i_ - 1, j_), i_mat_v_0)) Then
                        prop_expo = True
                    End If

                    If (Not prop_expo) AndAlso (IsSmaller(i_, Temp_0.GetUpperBound(0) + my_Mesh.ind_2) AndAlso IsEqual(my_Mesh.Tab_mesh_mat(i_ + 1, j_), i_mat_v_0)) Then
                        prop_expo = True
                    End If

                    prop_noex = False
                    If IsEqual(j_, Temp_0.GetUpperBound(1)) Then
                        prop_noex = True
                    End If

                    'Faces exterieures de la maille
                    If prop_expo Or prop_noex Then
                        'Emissivite de surface
                        If IsGreaterOrEqual(i_mat, i_mat_a_0) AndAlso IsSmallerOrEqual(i_mat, i_mat_a_1) Then
                            'val_epsilon = EN_Feu.EmissiviteAcier(Temp_0(i_, j_), myParamFeu.TypeSurface = cls_OptionsFeu.enu_TypeSurface.Galvanise)
                            val_epsilon = EN_Feu.EmissiviteAcier(Temp_0(i_y, j_), myParamFeu.TypeSurface = cls_OptionsFeu.enu_TypeSurface.Galvanise)
                        ElseIf IsEqual(i_mat, i_mat_c) Then
                            val_epsilon = myParamFeu.EmissivityC
                        End If

                        If prop_expo Then
                            'Face exposee
                            'h_net_ce = AlphaCInf * (TempG - Temp_0(i_, j_))    'densite de flux convectif
                            'h_net_re = EpsilonF * val_epsilon * SigmaSB * ((TempG + 273.0) ^ 4 - (Temp_0(i_, j_) + 273.0) ^ 4)    'densite de flux radiatif
                            h_net_ce = AlphaCInf * (TempG - Temp_0(i_y, j_))    'densite de flux convectif
                            h_net_re = EpsilonF * val_epsilon * SigmaSB * ((TempG + 273.0) ^ 4 - (Temp_0(i_y, j_) + 273.0) ^ 4)    'densite de flux radiatif
                            h_net_de = h_net_ce + h_net_re  'densite de flux net

                        ElseIf prop_noex Then
                            'Face non exposee
                            'h_net_cn = AlphaCSup * (myParamFeu.TempRef - Temp_0(i_, j_))    'densite de flux convectif
                            'h_net_rn = EpsilonF * val_epsilon * SigmaSB * ((myParamFeu.TempRef + 273.0) ^ 4 - (Temp_0(i_, j_) + 273.0) ^ 4)    'densite de flux radiatif
                            h_net_cn = AlphaCSup * (myParamFeu.TempRef - Temp_0(i_y, j_))    'densite de flux convectif
                            h_net_rn = EpsilonF * val_epsilon * SigmaSB * ((myParamFeu.TempRef + 273.0) ^ 4 - (Temp_0(i_y, j_) + 273.0) ^ 4)    'densite de flux radiatif
                            h_net_dn = h_net_cn + h_net_rn  'densite de flux net

                        End If

                    End If

                    '1er terme du denominateur de q_lb : face de gauche de la maille (i_,j_)
                    If IsEqual(i_, my_Mesh.ind_2) Then

                        dth_lb = 0.0
                        h_lb_1 = 0.0

                    Else

                        ii_ = i_ - 1
                        ii_y = i_y - 1
                        j_mat = my_Mesh.Tab_mesh_mat(ii_, j_)

                        If prop_expo AndAlso j_mat = i_mat_v_0 Then        '$ face exposee
                            'dth_lb = TempG - Temp_0(i_, j_)         '$ ecart de temperature entre les gaz chauds et la maille (i_-1,j_)
                            dth_lb = TempG - Temp_0(i_y, j_)         '$ ecart de temperature entre les gaz chauds et la maille (i_-1,j_)
                            h_lb_1 = dth_lb / h_net_de
                        ElseIf IsGreaterOrEqual(j_mat, i_mat_a_0) Then

                            'dth_lb = Temp_0(ii_, j_) - Temp_0(i_, j_) 'ecart de temperature entre les mailles (i_-1,j_) et (i_,j_)
                            dth_lb = Temp_0(ii_y, j_) - Temp_0(i_y, j_) 'ecart de temperature entre les mailles (i_-1,j_) et (i_,j_)

                            '%% Conductivite thermique de la maille (i-1,j_)

                            If IsSmallerOrEqual(j_mat, i_mat_a_1) Then  'acier
                                'lambda_Lb = EN_Feu.Conductivite_thermique_acier(Temp_0(ii_, j_))
                                lambda_Lb = EN_Feu.Conductivite_thermique_acier(Temp_0(ii_y, j_))
                            ElseIf IsEqual(j_mat, i_mat_c) Then 'beton
                                'lambda_Lb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(ii_, j_))
                                lambda_Lb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(ii_y, j_))
                            End If
                            h_lb_1 = 0.5 * my_Mesh.Tab_mesh_y(ii_) / lambda_Lb
                        End If
                    End If

                    '1er terme du denominateur de q_bb : face inferieure de la maille (i_,j_)
                    jj_ = j_ - 1
                    If prop_expo AndAlso (IsEqual(j_, Temp_0.GetLowerBound(1)) OrElse IsEqual(my_Mesh.Tab_mesh_mat(i_, jj_), i_mat_v_0)) Then
                        'dth_bb = TempG - Temp_0(i_, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                        dth_bb = TempG - Temp_0(i_y, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                        h_bb_1 = dth_bb / h_net_de
                    ElseIf IsGreaterOrEqual(my_Mesh.Tab_mesh_mat(i_, jj_), i_mat_a_0) Then
                        'dth_bb = Temp_0(i_, jj_) - Temp_0(i_, j_) 'ecart de temperature entre les mailles (i_,j_-1) et (i_,j_)
                        dth_bb = Temp_0(i_y, jj_) - Temp_0(i_y, j_) 'ecart de temperature entre les mailles (i_,j_-1) et (i_,j_)

                        'Conductivite thermique de la maille (i_,j_-1)
                        j_mat = my_Mesh.Tab_mesh_mat(i_, jj_)
                        If IsSmallerOrEqual(j_mat, i_mat_a_1) Then
                            'lambda_Bb = EN_Feu.Conductivite_thermique_acier(Temp_0(i_, jj_))
                            lambda_Bb = EN_Feu.Conductivite_thermique_acier(Temp_0(i_y, jj_))

                        ElseIf IsEqual(j_mat, i_mat_c) Then
                            'lambda_Bb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_, jj_))
                            lambda_Bb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_y, jj_))
                        End If

                        h_bb_1 = 0.5 * my_Mesh.Tab_mesh_z(jj_) / lambda_Bb
                    End If

                    '1er terme du denominateur de q_tb : face superieure de la maille (i_,j_)
                    If IsSmaller(j_, Temp_0.GetUpperBound(1)) Then

                        jj_ = j_ + 1
                        j_mat = my_Mesh.Tab_mesh_mat(i_, jj_)

                        If prop_expo AndAlso IsEqual(j_mat, i_mat_v_0) Then    'face superieure exposee
                            'dth_tb = TempG - Temp_0(i_, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                            dth_tb = TempG - Temp_0(i_y, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                            h_tb_1 = dth_tb / h_net_de
                        ElseIf IsGreaterOrEqual(j_mat, i_mat_a_0) Then
                            'dth_tb = Temp_0(i_, jj_) - Temp_0(i_, j_) 'ecart de temperature entre les mailles (i_,j_+1) et (i_,j_)
                            dth_tb = Temp_0(i_y, jj_) - Temp_0(i_y, j_) 'ecart de temperature entre les mailles (i_,j_+1) et (i_,j_)

                            'Conductivite thermique de la maille (i_,j_+1)                          '
                            If IsSmallerOrEqual(j_mat, i_mat_a_1) Then
                                'lambda_Tb = EN_Feu.Conductivite_thermique_acier(Temp_0(i_, jj_))
                                lambda_Tb = EN_Feu.Conductivite_thermique_acier(Temp_0(i_y, jj_))
                            ElseIf IsEqual(j_mat, i_mat_c) Then
                                'lambda_Tb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_, jj_))
                                lambda_Tb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_y, jj_))
                            End If

                            h_tb_1 = 0.5 * my_Mesh.Tab_mesh_z(jj_) / lambda_Tb
                        End If

                    ElseIf prop_noex Then    'face non exposee
                        'dth_tb = myParamFeu.TempRef - Temp_0(i_, j_) 'ecart de temperature entre l'air ambiant et la maille (i_,j_)
                        dth_tb = myParamFeu.TempRef - Temp_0(i_y, j_) 'ecart de temperature entre l'air ambiant et la maille (i_,j_)
                        If IsGreater(Math.Abs(dth_tb), 0) Then
                            h_tb_1 = dth_tb / h_net_dn
                        End If
                    End If

                    '1er terme du denominateur de q_rb : face de droite de la maille (i_,j_)
                    If IsSmaller(i_, my_Mesh.ind_1) Then

                        ii_ = i_ + 1
                        ii_y = i_y + 1
                        j_mat = my_Mesh.Tab_mesh_mat(ii_, j_)

                        If prop_expo AndAlso IsEqual(j_mat, i_mat_v_0) Then
                            'dth_rb = TempG - Temp_0(i_, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                            dth_rb = TempG - Temp_0(i_y, j_) 'ecart de temperature entre les gaz chauds et la maille (i_,j_)
                            h_rb_1 = dth_rb / h_net_de

                        ElseIf IsGreaterOrEqual(j_mat, i_mat_a_0) Then
                            'dth_rb = Temp_0(ii_, j_) - Temp_0(i_, j_) 'ecart de temperature entre les mailles (i_+1,j_) et (i_,j_)
                            dth_rb = Temp_0(ii_y, j_) - Temp_0(i_y, j_) 'ecart de temperature entre les mailles (i_+1,j_) et (i_,j_)

                            'Conductivite thermique de la maille (i_+1,j_)
                            If IsSmallerOrEqual(j_mat, i_mat_a_1) Then 'acier  
                                'lambda_Rb = EN_Feu.Conductivite_thermique_acier(Temp_0(ii_, j_))
                                lambda_Rb = EN_Feu.Conductivite_thermique_acier(Temp_0(ii_y, j_))
                            ElseIf IsEqual(j_mat, i_mat_c) Then
                                'lambda_Rb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(ii_, j_))
                                lambda_Rb = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(ii_y, j_))
                            End If

                            h_rb_1 = 0.5 * my_Mesh.Tab_mesh_y(ii_) / lambda_Rb
                        End If
                    Else
                        dth_rb = 0.0
                        h_rb_1 = 0.0
                    End If

                    '2e terme du denominateur de qi
                    If IsGreaterOrEqual(i_mat, i_mat_a_0) AndAlso IsSmallerOrEqual(i_mat, i_mat_a_1) Then
                        'lambda_ = EN_Feu.Conductivite_thermique_acier(Temp_0(i_, j_))
                        lambda_ = EN_Feu.Conductivite_thermique_acier(Temp_0(i_y, j_))
                        rho_ = cls_Acier.RHOACIER
                        'cp_ = EN_Feu.ChaleurSpecifiqueAcier(Temp_0(i_, j_))
                        cp_ = EN_Feu.ChaleurSpecifiqueAcier(Temp_0(i_y, j_))

                    ElseIf IsEqual(i_mat, i_mat_c) Then
                        'lambda_ = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_, j_))
                        'rho_ = EN_Feu.Masse_volumique_beton(lNormal, lRhoCVariable, lGeneration1, RhoC, Temp_0(i_, j_))
                        'cp_ = EN_Feu.Chaleur_specifique_beton(lNormal, val_U, lGeneration1, Temp_0(i_, j_))
                        lambda_ = EN_Feu.Conductivite_thermique_beton(lNormal, lANFrance, lGeneration1, Temp_0(i_y, j_))
                        rho_ = EN_Feu.Masse_volumique_beton(lNormal, lRhoCVariable, lGeneration1, RhoC, Temp_0(i_y, j_))
                        cp_ = EN_Feu.Chaleur_specifique_beton(lNormal, val_U, lGeneration1, Temp_0(i_y, j_))

                    End If

                    h_lb_2 = 0.5 * my_Mesh.Tab_mesh_y(i_) / lambda_
                    h_bb_2 = 0.5 * my_Mesh.Tab_mesh_z(j_) / lambda_
                    h_tb_2 = 0.5 * my_Mesh.Tab_mesh_z(j_) / lambda_
                    h_rb_2 = 0.5 * my_Mesh.Tab_mesh_y(i_) / lambda_

                    cst_lb = dth_lb / (h_lb_1 + h_lb_2)
                    cst_bb = dth_bb / (h_bb_1 + h_bb_2)
                    cst_tb = dth_tb / (h_tb_1 + h_tb_2)
                    cst_rb = dth_rb / (h_rb_1 + h_rb_2)

                    q_lb = my_Mesh.Tab_mesh_y(i_) * DeltaT * cst_lb
                    q_bb = my_Mesh.Tab_mesh_z(j_) * DeltaT * cst_bb
                    q_tb = my_Mesh.Tab_mesh_z(j_) * DeltaT * cst_tb
                    q_rb = my_Mesh.Tab_mesh_y(i_) * DeltaT * cst_rb

                    q_y = q_lb + q_rb
                    q_z = q_bb + q_tb

                    rho_cp = rho_ * cp_
                    k_1 = rho_cp * my_Mesh.Tab_mesh_y(i_) * my_Mesh.Tab_mesh_z(j_)
                    val_dth = (q_y + q_z) / k_1

                    'GiB 01/12/2025 : température supérieure ou égale à 20°C !
                    'Temp_1(i_, j_) = Math.Max(myParamFeu.TempRef, Temp_0(i_, j_) + val_dth)
                    'my_Mesh.Tab_mesh_temp(i_, j_) = Temp_1(i_, j_)
                    Temp_1(i_y, j_) = Math.Max(myParamFeu.TempRef, Temp_0(i_y, j_) + val_dth)
                    If IsGreater(Temp_1(i_y, j_), TempG) AndAlso IsEqual(TempG, EN_Feu.TemperatureGazISO(my_Time)) Then
                        Temp_1(i_y, j_) = Math.Min(Temp_1(i_y, j_), TempG)
                    End If
                    my_Mesh.Tab_mesh_temp(i_, j_) = Temp_1(i_y, j_)

                End If

            Next

        Next

        'Affectation éventuelle de températures des mailles hors [ind_0;ind_1] en fin de calcul

        If Not lTargetT Then

            With my_Mesh

                'Symétrie pour une poutre intérieure dont la largeur de la dalle est supérieure à delta_y de chaque côté de l'âme
                If Not IsEqual(.ind_0, .ind_2) Then
                    For j_ = 0 To .nb_cells_z - 1
                        For i_ = .ind_0 To .ind_2 - 1
                            ii_ = .Tab_mesh_y.GetUpperBound(0) - i_
                            .Tab_mesh_temp(i_, j_) = .Tab_mesh_temp(ii_, j_)
                        Next
                    Next
                End If

                If IsGreater(.ind_0, .Tab_mesh_y.GetLowerBound(0)) Then

                    For j_ = 0 To .nb_cells_z - 1
                        For i_ = 0 To .ind_0 - 1
                            .Tab_mesh_temp(i_, j_) = .Tab_mesh_temp(.ind_0, j_)
                        Next
                    Next

                End If

                If IsSmaller(.ind_1, .Tab_mesh_y.GetUpperBound(0)) Then
                    For j_ = 0 To .nb_cells_z - 1
                        For i_ = .ind_1 + 1 To .nb_cells_y - 1
                            .Tab_mesh_temp(i_, j_) = .Tab_mesh_temp(.ind_1, j_)
                        Next
                    Next
                End If

                'GiB 19/11/2025 : affectation de température des gaz chauds aux mailles du vide correspondantes

                For j_ = 0 To NbZ - 1

                    For i_ = 0 To NbY - 1

                        If .Tab_mesh_mat(i_, j_) = .MATVIDEOUVERT Then
                            .Tab_mesh_temp(i_, j_) = TempG
                        End If

                    Next

                Next

            End With

        End If

    End Sub


#End Region

End Class
