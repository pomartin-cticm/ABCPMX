...
#Region " Renseignement des paramètres de la poutre "
        ...
        myPoutre.Section.ProfilA.GenereProfileIPE450()
...
#Region " Renseignement des paramètres "
        ...
        myPoutre.Dalle.Bac.InitialiseCofraPlus60()
        myPoutre.Dalle.Bac.InitialiseCofraPlus60_075()
        myPoutre.Dalle.Bac.Orientation = cls_Bac.Enum_Orientation.Perpendiculaire
        myPoutre.Dalle.Bac.AppuiT = cls_Bac.EnuConfigTAppui.NervureEtBacContinus 'permet de prendre en compte le bac pour le calcul des armatures transversales
...
#Region " Renseignement des paramètres "
        ...
        myPoutre.lTraveeConsoleDroite = False
        myPoutre.LongueurTravee(0) = 3
        myPoutre.LongueurTravee(1) = 10
        myPoutre.lTremieGauche = False
        myPoutre.lTremieDroite = False
        myPoutre.lIntermediaire = True
        myPoutre.EntraxeD1 = 2.5
        ...
        myPoutre.Section.ProfilA.GenereProfileIPE360()
        myPoutre.Section.ProfilA.InitialiseProprietes()
        ...
        With myPoutre.Dalle
            ...
            .LitArma(0).lActive = True
            .LitArma(0).PhiS = 16 / 1000
            .LitArma(0).EspBar = 180 / 1000
            .LitArma(0).z_s = 20 / 1000 + .LitArma(0).PhiS / 2

            .AcierArmatures.FsK = 500
        End With

        For i As Integer = 1 To myPoutre.Dalle.LitArma.Count - 1 'on ne prend pas en compte qu'un seul lit d'arma
            myPoutre.Dalle.LitArma(i).lActive = False
        Next
        ...
        '# CHARGES
        Dim Qsurf() As Decimal = {0, 2, 16}

        myPoutre.InitialisePoidsPropres() '/!\ Les valeurs calculées par le logiciel ne sont pas exactement les mêmes que dans l'article. Elles seront recalculées à la main
        myPoutre.ChargesU("G2").QSurf(0) = Qsurf(1) * 1000
        myPoutre.ChargesU("G2").QSurf(1) = Qsurf(1) * 1000
        myPoutre.ChargesU("Q1").QSurf(0) = Qsurf(2) * 1000
...
#Region " VALIDATION : Analyse de la poutre "
        ...
        Valeur = VG1max
        ValRef = 29.96 * 10 ^ 3 'valeur calculée à la main
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx))

        Valeur = VG1min
        ValRef = -35.88 * 10 ^ 3 'valeur calculée à la main
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx))
        ...
        'Q1#2 : sur la travée centrale

        myPoutre.ChargesA(3).EnveloppesMoments(MQmax, iNodeMax, MQmin, iNodeMin)
        myPoutre.ChargesA(3).EnveloppesTranchants(VQmax, iNodeMax, VQmin, iNodeMin)
        ...
        myPoutre.ChargesA(4).EnveloppesMoments(MQmax, iNodeMax, MQmin, iNodeMin)
        myPoutre.ChargesA(4).EnveloppesTranchants(VQmax, iNodeMax, VQmin, iNodeMin)
        ...
        Valeur = MEdMax(iELU)
        ValRef = 62.09 * 10 ^ 3
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx * 10))
        Valeur = MEdMin(iELU)
        ValRef = -340.37 * 10 ^ 3
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx))
...
#Region " VALIDATION : Degré de connection minimal (ELU) "
        ...
        Valeur = myPoutre.VerifMixte(0).DegConnexMin(1)
        ValRef = 0.505
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx))
...
#Region " VALIDATION : Résistance à la flexion (ELU) "
        ...
        '# Critère par travée et par ELU

        '## ELU01
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(0, 0)
        ValRef = 0.6596
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(0, 1)
        ValRef = 1.307
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)

        '## ELU02
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(1, 0)
        ValRef = 0.1364
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(1, 1)
        ValRef = 1.575
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)

        '## ELU03
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(2, 0)
        ValRef = 0.6596
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(2, 1)
        ValRef = 0.6596
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)

        'A L'ELU en cisaillement

        Valeur = myPoutre.VerifMixte(0).CritereV.Resistance(iNodeMMax)
        ValRef = 720.2 * 1000
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaCMAx))
...
#End Region
    ...
    <TestMethod> Public Sub TU_MV_TESTC05B()

        'Cas test symétrique du C05 du MV, permettant de gérer les poutres mixtes avec console
        '
...
#Region " Renseignement des paramètres "
        ...
        myPoutre.InitialisePoidsPropres() '/!\ Les valeurs calculées par le logiciel ne sont pas exactement les mêmes que dans l'article. Elles seront recalculées à la main
        myPoutre.ChargesU("G2").QSurf(2) = Qsurf(1) * 1000
        myPoutre.ChargesU("G2").QSurf(1) = Qsurf(1) * 1000
        myPoutre.ChargesU("Q1").QSurf(2) = Qsurf(2) * 1000
        myPoutre.ChargesU("Q1").QSurf(1) = Qsurf(2) * 1000
...
#Region " VALIDATION : Résistance à la flexion (ELU) "
        ...
        '## ELU01
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(0, 2)
        ValRef = 0.6596
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(0, 1)
        ValRef = 1.307
        ...
        '## ELU02
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(1, 2)
        ValRef = 0.1364
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(1, 1)
        ValRef = 1.575
        ...
        '## ELU03
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(2, 2)
        ValRef = 0.6596
        Assert.IsTrue(Math.Abs(Valeur - ValRef) <= DeltaCMAx)
        Valeur = myPoutre.VerifMixte(0).CritereM.CritereCombiT(2, 1)
        ValRef = 0.6596
...
#End Region
    ...
    <TestMethod> Public Sub TU_MV_TESTC06()
        ...
        'Cas test C06 du MV, permettant de gérer les poutres mixtes avec un dalle pleine
        '
...
#End Region
    ...
    <TestMethod> Public Sub TU_MV_TESTC07()


        'Cas test C07 du MV, permettant de gérer les poutres mixtes avec un dalle pleine
        '
...
#Region " Renseignement des paramètres "
        ...
        myPoutre.lIntermediaire = True
        myPoutre.EntraxeD1 = 5
        myPoutre.EntraxeD2 = 5
        ...
        myPoutre.Dalle.Bac.InitialiseCofraplus220()
        myPoutre.Dalle.Bac.Orientation = cls_Bac.Enum_Orientation.Perpendiculaire
        ...
        For itravee As Integer = myPoutre.IndicePremiereTravee To myPoutre.IndiceDerniereTravee
            ...
            myPoutre.NrTransZone(itravee, 0) = 2
            'myPoutre.Espacement_Bac_TransZone(itravee, 0) = 1
            myPoutre.EspacementZone(itravee, 0) = 0.25
            myPoutre.LongueurZone(itravee, 0) = myPoutre.LongueurTravee(itravee)
        Next
        myPoutre.lAutomaticDesign = False

        '# MATERIAUX
        myPoutre.Section.Acier.InitialiseAcierS420EC3()

        myPoutre.Dalle.Goujons.Fu = 450

        '# CHARGES
        Dim Qsurf() As Decimal = {0, 1, 3}
...
#Region " VALIDATION : Analyse de la poutre "
        ...
        Valeur = VG1min
        ValRef = -141.04 * 10 ^ 3 'valeur calculée à la main
        Assert.IsTrue(IsEqual(Valeur, ValRef, DeltaVMAx))
